...
for (col in numeric_cols) {
  ...
  print(ggplot(data = df, aes(x = Year, y = eval(parse(text = col)))) + geom_line(aes(colour = Country)))
}
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    output$LifePlot <- renderPlot({
      ggplot(data = df_filter(), aes(x = Year, y = eval(parse(text = "Life.expectancy")))) + geom_line(aes(colour = Country))
    })
  })
```
